...
rotateFirst = True     # If True, the constellation starts rotated by 1 movement defined by ndeltas
...
# nnpath      = './Results/NNs/kepler-f=0.8/qNetwork_2GTs.h5'
# nnpathTarget= './Results/NNs/kepler-f=0.8/qTarget_2GTs.h5'
...
class DataBlock:
    ...
    def __init__(self, source, destination, ID, creationTime):
        ...
        self.newPath = []
        self.QPath   = []
        self.queue   = []  #记录数据块在每个节点选择下一跳时，所观察到的下一跳发送队列长度
        self.queueTime= []  # 每个节点的实际排队时间
        self.oldState  = None
        self.oldAction = None
        # self.oldReward = None
...
def createGraph(earth, matching='Greedy'):
    ...
    for GT in earth.gateways:
        if GT.linkedSat[1]:
            ...
            slant_range = GT.linkedSat[0],          # slant range  卫星和网关的距离
            invDataRate = 1/GT.dataRate,            # Inverse of dataRate  数据率的倒数
            dataRateOG = GT.dataRate,               # original shannon dataRate  原始数据率
