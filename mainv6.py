...
GAT_BUFFER_FEATURE_INDEX = GAT_FEATURE_NAMES.index('buffer_utilization')
GAT_TREND_FEATURE_INDEX = GAT_FEATURE_NAMES.index(
    'multiscale_queue_trend'
)
...
GAT_REWARD_PROGRESS_WEIGHT =0.5 # 0.50
GAT_REWARD_CONGESTION_WEIGHT =0.1 #  0.10
GAT_REWARD_QUEUE_WEIGHT =0.25   # 0.25
GAT_REWARD_HOP_WEIGHT = 0.15  #  0.05
GAT_REWARD_REPEAT_WEIGHT =0.1  #  0.10

GAT_HOP_PENALTY = -1.0

GAT_CONGESTION_ROOT_QUEUE_WEIGHT = 0.45
GAT_CONGESTION_NEXT_BUFFER_WEIGHT = 0.40
GAT_CONGESTION_TREND_WEIGHT = 0.15
GAT_CONGESTION_THRESHOLD = 0.75  #改为0.75提高阈值
GAT_CONGESTION_STEEPNESS = 12.0
...
GAT_BACKTRACK_PENALTY_MULTIPLIER = 2.0  #2/3/4
...
bufferSize  = 10000        # bufferSize samples are used to train the network
    ...
    def receiveBlock(self, block, propTime):
        ...
        if ((self.QLearning) or (self.orbPlane.earth.DDQNA is not None) or (self.DDQNA is not None)):
            if len(block.QPath) > 3: # the block does not come from a gateway
                prevSat = getPreviousSatelliteFromQPath(
                    block,
                    self,
                    self.orbPlane.earth
                )
                if self.QLearning:
                    nextHop = self.QLearning.makeAction(block, self, self.orbPlane.earth.gateways[0].graph, self.orbPlane.earth, prevSat=prevSat)
                elif self.DDQNA:
                    nextHop = self.DDQNA.makeDeepAction(block, self, self.orbPlane.earth.gateways[0].graph, self.orbPlane.earth, prevSat=prevSat)
                else:
                    nextHop = self.orbPlane.earth.DDQNA.makeDeepAction(block, self, self.orbPlane.earth.gateways[0].graph, self.orbPlane.earth, prevSat=prevSat)
            else:
                if self.QLearning:
...
class Earth:
            ...
            for sat in plane.sats:
                ...
                if self.pathParam == 'Q-Learning':
                    ...
                    sat.QLearning.linkedSats =  {'U': linkedSats['U'],
                                    ...
                                    'R': linkedSats['R'],
                                    'L': linkedSats['L']}
                elif self.pathParam == 'Deep Q-Learning' or self.pathParam =='GAT-DQN':
    ...
    def moveConstellation(self, env, deltaT=3600, getRates = False):
        ...
        while True:
            ...
            if self.pathParam == 'Deep Q-Learning' or self.pathParam == 'Q-Learning' or self.pathParam == 'GAT-DQN':
    ...
    def plotMap(self, plotGT = True, plotSat = True, path = None, bottleneck = None, save = False, ID=None, time=None, edges=False, arrow_gap=0.008, outputPath='', paths=None, fileName="map.png", n = None):
        ...
        if paths is not None:
            link_usage = calculate_link_usage([block.QPath for block in paths]) if pathing in RL_PATHINGS else calculate_link_usage([block.path for block in paths])
...
class hyperparam:
    def __init__(self, pathing):
        ...
        if pathing == 'GAT-DQN':
            ...
            self.gatCongestionNextWeight = (
                GAT_CONGESTION_NEXT_BUFFER_WEIGHT
            )
            self.gatCongestionTrendWeight = GAT_CONGESTION_TREND_WEIGHT
            self.gatCongestionThreshold = GAT_CONGESTION_THRESHOLD
            self.gatCongestionSteepness = GAT_CONGESTION_STEEPNESS
            self.gatQueueTimeScale = GAT_QUEUE_TIME_SCALE_S
            self.gatArrivalBonus = GAT_ARRIVAL_BONUS
            self.gatRepeatPenalty = GAT_REPEAT_PENALTY
            self.gatBacktrackPenaltyMultiplier = (
                GAT_BACKTRACK_PENALTY_MULTIPLIER
            )
...
class DenseGraphAttention(keras.layers.Layer):
    def __init__(self, units, heads=4, concat=True, activation='elu', **kwargs):
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.add_weight(
            name='kernel',
            shape=(self.heads, input_dim, self.units),
    ...
    def call(self, inputs):
        ...
        # 即使 batch 内有补齐节点，也给其保留 self-loop，避免 softmax 出现 NaN。
        identity = tf.eye(num_nodes, dtype=tf.float32)[None, :, :]
        adjacency = tf.maximum(adjacency, identity)
        edge_mask = adjacency > 0.0

        # h: [B, N, heads, units]
        h = tf.einsum('bnf,hfo->bnho', x, self.kernel)

        source_score = tf.einsum('bnho,ho->bnh', h, self.attn_src)
        target_score = tf.einsum('bnho,ho->bnh', h, self.attn_dst)

        # scores: [B, N, N, heads]
        scores = source_score[:, :, None, :] + target_score[:, None, :, :]
        ...
        scores = tf.where(
            edge_mask[:, :, :, None],
            scores,
            tf.ones_like(scores) * negative_inf
        )

        attention = tf.nn.softmax(scores, axis=2)

        # [B, N, heads, units]
        output = tf.einsum('bijh,bjho->biho', attention, h)
...
@keras.utils.register_keras_serializable(package='gat_dqn')
class RootNodeReadout(keras.layers.Layer):
    def call(self, node_embeddings):
        return node_embeddings[:, 0, :]


@keras.utils.register_keras_serializable(package='gat_dqn')
class ActionNodeGather(keras.layers.Layer):
    def call(self, inputs):
        node_embeddings, action_nodes = inputs

        # 无效动作的节点索引为 -1；先安全地替换为根节点，
        # 真正的无效动作由 action_mask 在选动作和 Bellman target 中屏蔽。
        safe_indices = tf.maximum(
            tf.cast(action_nodes, tf.int32),
            tf.zeros_like(tf.cast(action_nodes, tf.int32))
        )
...
def create_gat_q_network(k_hops, feature_dim, hidden_dim, heads):
    ...
    adjacency = keras.Input(
        shape=(None, None),
        dtype=tf.float32,
        name='adjacency'
    )
    node_mask = keras.Input(
        shape=(None,),
        dtype=tf.float32,
        name='node_mask'
    )
    ...
    # k 层 GAT：根节点可获得 k-hop 范围的信息。
    for layer_index in range(k_hops):
        is_last_layer = layer_index == k_hops - 1

        x = DenseGraphAttention(
            units=hidden_dim,
            heads=heads,
    ...
    q_values = keras.layers.TimeDistributed(
        keras.layers.Dense(1, activation='linear'),
        name='q_value'
    )(action_input)

    q_values = keras.layers.Flatten(name='q_values')(q_values)  # [B, 4]

    return keras.Model(
        inputs=[node_features, adjacency, node_mask, action_nodes],
        outputs=q_values,
        name='GAT_DQN'
    )
...
class GATDQNAgent:
    def __init__(self, NGT, hyperparams, earth, sat_ID=None):
        ...
        self.kHops = hyperparams.gatKHops
        self.featureDim = hyperparams.gatFeatureDim
        self.hiddenDim = hyperparams.gatHiddenDim
        self.heads = hyperparams.gatHeads
        ...
        self.gatCongestionThreshold = hyperparams.gatCongestionThreshold
        self.gatCongestionSteepness = hyperparams.gatCongestionSteepness
        self.gatQueueTimeScale = hyperparams.gatQueueTimeScale
        self.gatArrivalBonus = hyperparams.gatArrivalBonus
        self.gatRepeatPenalty = hyperparams.gatRepeatPenalty
        self.gatBacktrackPenaltyMultiplier = (
            hyperparams.gatBacktrackPenaltyMultiplier
        )
        ...
        if self.importQ:
            global gat_nnpath
            global gat_nnpathTarget

            self.qNetwork = keras.models.load_model(gat_nnpath)
            ...
            self.qNetwork = create_gat_q_network(
                self.kHops,
                self.featureDim,
                self.hiddenDim,
                self.heads
            ...
            self.qTarget = create_gat_q_network(
                self.kHops,
                self.featureDim,
                self.hiddenDim,
                self.heads
            ...
            # 新建 target 网络必须先与 Q 网络同步。
            self.qTarget.set_weights(self.qNetwork.get_weights())

            if sat_ID is None:
                print('----------------------------------')
                print(f'GAT-DQN created: k={self.kHops}, '
                      f'heads={self.heads}, hidden={self.hiddenDim}')
                self.qNetwork.summary()
                print('----------------------------------')
    ...
    def _masked_q_values(self, q_values, action_mask):
        action_mask = tf.cast(action_mask, tf.float32)

        return tf.where(
            action_mask > 0.0,
            q_values,
            tf.ones_like(q_values) * tf.cast(-1e9, q_values.dtype)
        )

    def getNextHop(self, graph_state, block, sat, earth):
        valid_actions = np.flatnonzero(graph_state['action_mask'] > 0.0)

        if valid_actions.size == 0:
            return -1, -1

        destination_sat = block.destination.linkedSat[1]

        # 保留原 noPingPong 策略，但只允许真实存在的根节点一跳边。
        if noPingPong and destination_sat is not None:
            for action_index, node_index in enumerate(
                graph_state['action_nodes']
            ):
                if node_index < 0:
                    continue

                # Every policy branch must respect the same final action mask.
                if graph_state['action_mask'][action_index] <= 0.0:
                    continue

                if graph_state['node_ids'][node_index] == destination_sat.ID:
                    destination = findByID(
                        earth,
                        graph_state['node_ids'][node_index]
                    )

                    return [
                        destination.ID,
                        math.degrees(destination.longitude),
                        math.degrees(destination.latitude)
                    ], action_index

        # epsilon-greedy：随机动作只从真实可连接方向中选择。
        if explore and random.uniform(0, 1) < self.alignEpsilon(self.step, sat):
            action_index = int(random.choice(valid_actions))

        else:
            model_inputs, action_mask = pack_gat_states([graph_state])

            q_values = self.qNetwork(
                model_inputs,
                training=False
            ).numpy()[0]

            q_values[action_mask[0] <= 0.0] = -np.inf
            action_index = int(np.argmax(q_values))

        node_index = graph_state['action_nodes'][action_index]
        destination = findByID(
            earth,
            graph_state['node_ids'][node_index]
        )
    ...
    def _action_congestion_inputs(self, state, action):
        """Read the selected action's congestion snapshot from replay state."""
        if state is None or 'x' not in state or 'action_nodes' not in state:
            raise ValueError('A GAT congestion reward requires a graph state.')

        action = int(action)
        if action < 0 or action >= self.actionSize:
            raise ValueError(f'Invalid GAT reward action index: {action}')

        node_features = np.asarray(state['x'])
        action_nodes = np.asarray(state['action_nodes'])
        next_node_index = int(action_nodes[action])

        if next_node_index < 0 or next_node_index >= node_features.shape[0]:
            raise ValueError(
                f'GAT reward action {action} has no valid next-hop node.'
            )

        root_queue = node_features[
            0,
            GAT_ACTION_QUEUE_FEATURE_INDICES[action]
        ]
        next_buffer = node_features[
            next_node_index,
            GAT_BUFFER_FEATURE_INDEX
        ]
        next_trend = node_features[
            next_node_index,
            GAT_TREND_FEATURE_INDEX
        ]
    ...
    def _congestion_reward(self, state, action):
        ...
        root_queue, next_buffer, next_trend = (
            self._action_congestion_inputs(state, action)
        )

        root_queue = float(np.clip(root_queue, 0.0, 1.0))
        next_buffer = float(np.clip(next_buffer, 0.0, 1.0))
        positive_trend = float(np.clip(max(0.0, next_trend), 0.0, 1.0))

        congestion_risk = np.clip(
            self.gatCongestionRootWeight * root_queue
            + self.gatCongestionNextWeight * next_buffer
            + self.gatCongestionTrendWeight * positive_trend,
            0.0,
            1.0
        )

        low = self._sigmoid(
            -self.gatCongestionSteepness * self.gatCongestionThreshold
        )
        high = self._sigmoid(
            self.gatCongestionSteepness
            * (1.0 - self.gatCongestionThreshold)
        )
        value = self._sigmoid(
            self.gatCongestionSteepness
            * (float(congestion_risk) - self.gatCongestionThreshold)
        )
        normalized_penalty = np.clip(
            (value - low) / (high - low),
            0.0,
            1.0
        )
    ...
    def _queue_delay_reward(self, block):
        """Bounded penalty for the queue delay actually experienced."""
        if not getattr(block, 'queueTime', None):
            return 0.0

        queue_time = float(block.queueTime[-1])
        if not math.isfinite(queue_time):
            return -1.0

        queue_time = max(queue_time, 0.0)
        scaled_delay = queue_time / self.gatQueueTimeScale
        if scaled_delay >= 20.0:
            return -1.0
    ...
    def _is_repeated_satellite(block, satellite):
        ...
        previous_hops = getattr(block, 'QPath', [])
        previous_hops = previous_hops[:max(0, len(previous_hops) - 2)]
        return any(
            isinstance(hop, (list, tuple))
            and hop
            and hop[0] == satellite.ID
            for hop in previous_hops
        )

    @staticmethod
    def _is_immediate_backtrack(block, prev_sat, next_sat):
        """Return whether the transition reverses the preceding hop."""
        if prev_sat is None or next_sat is None:
            return False

        prev_index = getCurrentSatelliteQPathIndex(block, prev_sat)
        if prev_index is None or prev_index <= 0:
            return False

        sat_before_prev = block.QPath[prev_index - 1]
        return (
            isinstance(sat_before_prev, (list, tuple))
            and sat_before_prev
            and sat_before_prev[0] == next_sat.ID
        )

    def _repeat_reward(self, block, prev_sat, next_sat):
        """Penalize an immediate a->b->a reversal more than other revisits."""
        if not self._is_repeated_satellite(block, next_sat):
            return 0.0

        repeat_penalty = self.gatRepeatPenalty
        if self._is_immediate_backtrack(block, prev_sat, next_sat):
            repeat_penalty *= self.gatBacktrackPenaltyMultiplier

        return repeat_penalty

    def _selected_next_satellite(self, block):
        """Resolve the neighbor selected in oldState without using live links."""
        state = block.oldState
        action = int(block.oldAction)
        node_index = int(state['action_nodes'][action])
        if node_index < 0:
            return None
        node_id = state['node_ids'][node_index]
        return findByID(self.earth, node_id)
    ...
    def _dense_reward(
        ...
        repeat_reward = self._repeat_reward(
            block,
            prev_sat,
            next_sat
        )
    ...
    def computeDropReward(
        ...
        # score the intended neighbor and do not reuse the preceding hop's delay.
        if (
            reason == 'no_route'
            and drop_sat is not None
            and decision_sat.ID == drop_sat.ID
        ):
    ...
    def makeDeepAction(self, block, sat, graph, earth, prevSat=None):
        ...
        nextHop, action_index = self.getNextHop(
            graph_state,
            block,
            sat,
            earth
        ...
        if prevSat is not None and getattr(block, 'oldState', None) is not None:
            reward = self._transition_reward(block, prevSat, sat)
    ...
    def alignQTarget(self):
        if self.hardUpd:
            ...
            for target_variable, online_variable in zip(
                self.qTarget.trainable_variables,
                self.qNetwork.trainable_variables
            ):
                target_variable.assign(
                    target_variable * (1.0 - self.tau)
                    + online_variable * self.tau
                )
    ...
    def train(self, sat, earth):
        ...
        mini_batch = self.experienceReplay.getBatch(self.batchS)

        states = [sample[0] for sample in mini_batch]
        actions = np.asarray(
            [sample[1] for sample in mini_batch],
            dtype=np.int32
        )
        rewards = np.asarray(
            [sample[2] for sample in mini_batch],
            dtype=np.float32
        )
        next_states = [sample[3] for sample in mini_batch]
        dones = np.asarray(
            [sample[4] for sample in mini_batch],
            dtype=np.float32
        )

        model_inputs, _ = pack_gat_states(states)
        ...
        future_value = tf.where(
            has_next_action,
            future_value,
            tf.zeros_like(future_value)
        )

        targets = (
            tf.convert_to_tensor(rewards, dtype=tf.float32)
            + self.gamma
            * (1.0 - tf.convert_to_tensor(dones, dtype=tf.float32))
            * future_value
        )
        targets = tf.stop_gradient(targets)

        with tf.GradientTape() as tape:
            q_values = self.qNetwork(model_inputs, training=True)

            selected_q = tf.gather(
                q_values,
            ...
            loss = tf.reduce_mean(
                self.loss_function(targets, selected_q)
            )
        ...
        self.optimizer.apply_gradients(
            zip(gradients, self.qNetwork.trainable_variables)
        )

        loss_value = float(loss.numpy())
        earth.loss.append([loss_value, sat.env.now])
        earth.trains.append([sat.env.now])

        if stopLoss and len(earth.loss) > nLosses:
            recent_losses = [item[0] for item in earth.loss[-nLosses:]]
            average_loss = sum(recent_losses) / nLosses
            earth.lossAv.append(average_loss)

            if average_loss < lThreshold:
                global TrainThis
                TrainThis = False
                print('----------------------------------')
                print('STOP LOSS ACTIVATED')
                print(f'Last {nLosses} losses: {recent_losses}')
                print(f'Simulation time: {sat.env.now}')
                print('----------------------------------')
...
def initialize(env, popMapLocation, GTLocation, distance, inputParams, movementTime, totalLocations, outputPath, matching='Greedy'):
    ...
    # gat init
    if pathing in RL_PATHINGS:
        hyperparams = hyperparam(pathing)

    if pathing in DEEP_RL_PATHINGS:
        AgentClass = GATDQNAgent if pathing == 'GAT-DQN' else DDQNAgent

        if not onlinePhase:
            earth.DDQNA = AgentClass(
                len(earth.gateways),
                hyperparams,
                earth
            )
            ...
            print('----------------------------------')
            print('Creating satellite agents...')

            for plane in earth.LEO:
                for sat in plane.sats:
                    sat.DDQNA = AgentClass(
                        len(earth.gateways),
                        hyperparams,
                        earth,
                        sat.ID
...
def _live_directional_neighbours(sat, graph, earth):
    """
    返回真正存在于当前动态图中的 U/D/R/L 邻居。

    规则：
    1. 卫星属性给出候选方向邻居；
    2. 必须满足 graph.has_edge(sat.ID, neighbour.ID)；
    3. 同一卫星不会在同一个节点的多个方向重复出现。
    """
    raw_neighbours = getDeepLinkedSats(sat, graph, earth)
    result = {}
    seen_ids = set()

    for direction in GAT_ACTIONS:
        neighbour = raw_neighbours[direction]

        if neighbour is None:
            result[direction] = None
            continue

        if neighbour.ID == sat.ID:
            result[direction] = None
            continue

        if not graph.has_node(neighbour.ID):
            result[direction] = None
            continue

        if not graph.has_edge(sat.ID, neighbour.ID):
            result[direction] = None
            continue

        # 极区或动态更新异常时，避免一个物理卫星在多个方向重复。
...
def _visited_satellite_counts(block, current_sat):
    """Count actually visited nodes up to the packet's resident satellite.

    QPath can contain a planned next hop while a packet is waiting in a send
    buffer.  Only the prefix ending at ``current_sat`` has been traversed, so
    using QPath[:-1] would incorrectly classify that planned hop as visited.
    """
    current_index = getCurrentSatelliteQPathIndex(block, current_sat)
    if current_index is None:
        return {}, None

    visited_counts = {}
    for step in block.QPath[:current_index + 1]:
        if not isinstance(step, (list, tuple)) or not step:
            continue

        node_id = step[0]
        visited_counts[node_id] = visited_counts.get(node_id, 0) + 1

    previous_id = None
    if current_index > 0:
        previous_step = block.QPath[current_index - 1]
        if isinstance(previous_step, (list, tuple)) and previous_step:
            previous_id = previous_step[0]

    return visited_counts, previous_id


def _history_aware_action_mask(
    block,
    root_sat,
    destination_sat,
    node_ids,
    ...
    """Mask revisits and relax safely only when no loop-free action exists."""
    action_nodes = np.asarray(action_nodes, dtype=np.int32)
    raw_action_mask = (action_nodes >= 0).astype(np.float32)
    action_mask = raw_action_mask.copy()
    visited_counts, previous_id = _visited_satellite_counts(
        block,
        root_sat
    )

    for action_index, node_index in enumerate(action_nodes):
        if node_index < 0:
            continue

        neighbour_id = node_ids[int(node_index)]

        # Reaching the satellite currently serving the destination GT must
        # never be blocked by stale path history.
        if neighbour_id == destination_sat.ID:
            continue

        if visited_counts.get(neighbour_id, 0) > 0:
            action_mask[action_index] = 0.0

    history_fallback = False

    # A dynamic topology can leave only previously visited physical
    # neighbours.  Preserve delivery by exposing only the least harmful
    # fallback actions instead of restoring every visited action.
    if not np.any(action_mask) and np.any(raw_action_mask):
        physical_actions = list(np.flatnonzero(raw_action_mask > 0.0))
        non_backtrack_actions = [
            action_index
            for action_index in physical_actions
            if node_ids[int(action_nodes[action_index])] != previous_id
        ]
        candidates = non_backtrack_actions or physical_actions
        minimum_visits = min(
            visited_counts.get(
                node_ids[int(action_nodes[action_index])],
                0
            )
        ...
        action_mask[fallback_actions] = 1.0
        history_fallback = True

    return raw_action_mask, action_mask, history_fallback


def build_gat_k_hop_state(block, root_sat, graph, earth, k_hops):
    """
    构造以 root_sat 为中心的唯一 k-hop 卫星子图。

    返回的节点顺序：
    - 节点 0 始终是当前卫星 root_sat；
    - 后续节点按 BFS 和 U/D/R/L 的固定顺序加入；
    - 卫星 ID 去重。
    """
    destination_sat = block.destination.linkedSat[1]
    if destination_sat is None:
        print(f'{block.destination} has no linked satellite :(')
        return None

    k_hops = max(1, int(k_hops))
    ...
    for _ in range(k_hops):
        ...
        for current_sat in frontier:
            ...
            for direction in GAT_ACTIONS:
                ...
                if neighbour.ID in node_to_index:
                    continue

                node_to_index[neighbour.ID] = len(nodes)
    ...
    # 诱导子图邻接矩阵：只有已在 k-hop 子图中的真实 ISL 才置 1。
    ...
    for action_index, direction in enumerate(GAT_ACTIONS):
        neighbour = root_neighbours[direction]

        if neighbour is not None:
            action_nodes[action_index] = node_to_index[neighbour.ID]

    node_ids = tuple(sat.ID for sat in nodes)
    raw_action_mask, action_mask, history_fallback = (
        _history_aware_action_mask(
            block,
            root_sat,
            destination_sat,
            node_ids,
    ...
    assert node_features.shape == (num_nodes, GAT_FEATURE_DIM)
    ...
    return {
        ...
        'action_mask': action_mask,         # [4]
        'raw_action_mask': raw_action_mask, # [4], physical-link mask
        'history_masked_count': int(np.sum(
            (raw_action_mask > 0.0) & (action_mask <= 0.0)
        )),
        'history_fallback': history_fallback
    }


def pack_gat_states(states):
    """
    把经验回放中不同大小的图补齐成一个 batch。

    返回：
    inputs = [X, A, node_mask, action_nodes]
    action_mask
    """
    batch_size = len(states)
    max_nodes = max(state['x'].shape[0] for state in states)

    x = np.zeros(
        (batch_size, max_nodes, GAT_FEATURE_DIM),
        dtype=np.float32
    )
    adjacency = np.zeros(
    ...
    action_nodes = np.full((batch_size, 4), -1, dtype=np.int32)
    action_mask = np.zeros((batch_size, 4), dtype=np.float32)

    for batch_index, state in enumerate(states):
        num_nodes = state['x'].shape[0]

        x[batch_index, :num_nodes] = state['x']
        adjacency[batch_index, :num_nodes, :num_nodes] = state['adj']
        node_mask[batch_index, :num_nodes] = 1.0
        action_nodes[batch_index] = state['action_nodes']
        action_mask[batch_index] = state['action_mask']

    return [x, adjacency, node_mask, action_nodes], action_mask
...
def saveHyperparams(outputPath, inputParams, hyperparams):
    ...
    if hyperparams.pathing == 'GAT-DQN':
        gat_reward_params = [
            ...
            + str(hyperparams.gatCongestionNextWeight),
            'GAT congestion trend weight: '
            + str(hyperparams.gatCongestionTrendWeight),
            'GAT congestion threshold: '
            + str(hyperparams.gatCongestionThreshold),
            'GAT congestion steepness: '
            + str(hyperparams.gatCongestionSteepness),
            'GAT queue time scale (s): '
            + str(hyperparams.gatQueueTimeScale),
            'GAT arrival bonus: '
            + str(hyperparams.gatArrivalBonus),
            'GAT repeat penalty: '
            + str(hyperparams.gatRepeatPenalty),
            'GAT backtrack penalty multiplier: '
            + str(hyperparams.gatBacktrackPenaltyMultiplier),
    ...
    hyperparams.extend(gat_reward_params)
...
# 保存 GAT 网络
def saveGATNetworks(outputPath, earth):
    print('Saving GAT-DQN networks at: ' + outputPath)
    os.makedirs(outputPath, exist_ok=True)

    if not onlinePhase:
        earth.DDQNA.qNetwork.save(
            outputPath + 'gat_qNetwork_'
            + str(len(earth.gateways))
            + 'GTs.keras'
        )
        earth.DDQNA.qTarget.save(
            outputPath + 'gat_qTarget_'
            + str(len(earth.gateways))
            + 'GTs.keras'
        )
        ...
        for plane in earth.LEO:
            for sat in plane.sats:
                sat.DDQNA.qNetwork.save(
                    outputPath + sat.ID + '_gat_qNetwork_'
                    + str(len(earth.gateways))
                    + 'GTs.keras'
                )
                sat.DDQNA.qTarget.save(
                    outputPath + sat.ID + '_gat_qTarget_'
                    + str(len(earth.gateways))
                    + 'GTs.keras'
                )
...
def plotCongestionMap(self, paths, outPath, GTnumber, plot_separately=True):
    ...
    unique_routes = {}
    for block in paths:
        p = block.QPath if pathing in RL_PATHINGS else block.path
...
def RunSimulation(GTs, inputPath, outputPath, populationData, radioKM):
    ...
    for GTnumber in GTs:
        ...
        if firstGT:
            ...
            # 当前 GTnumber 会加载上一个 GTnumber 训练后保存的模型。
            if pathing == 'GAT-DQN':
                gat_nnpath = (
                    f'{outputPath}/NNs/gat_qNetwork_{GTnumber - 1}GTs.keras'
                )
                gat_nnpathTarget = (
                    f'{outputPath}/NNs/gat_qTarget_{GTnumber - 1}GTs.keras'
                )

            else:
                nnpath = (
                    f'{outputPath}/NNs/qNetwork_{GTnumber - 1}GTs.h5'
                )

                if ddqn:
                    nnpathTarget = (
                        f'{outputPath}/NNs/qTarget_{GTnumber - 1}GTs.h5'
                    )
        if len(GTs)>1:
        ...
        if testType == "Rates":
            ...
            if pathing in RL_PATHINGS:
                ...
                if not onlinePhase:
                    eps = earth1.DDQNA.epsilon if pathing in DEEP_RL_PATHINGS else earth1.epsilon
                else:
                    eps = earth1.LEO[0].sats[0].DDQNA.epsilon if pathing in DEEP_RL_PATHINGS else earth1.epsilon
                # save epsilons
                if Train:
        ...
        elif pathing == 'GAT-DQN':
            saveGATNetworks(outputPath + '/NNs/', earth1)
